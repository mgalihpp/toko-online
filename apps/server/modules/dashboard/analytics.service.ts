...
export class AnalyticsService {
   ...
   */
  async getSalesSummary(period: "day" | "week" | "month" | "year" = "month") {
    const now = new Date();
    const { startDate, previousStartDate, previousEndDate } =
      this.getPeriodDates(period, now);
  ...
  async getSalesByCategory() {
    ...
    for (const item of orderItems) {
      const categoryName =
        item.variant?.product?.category?.name || "Tanpa Kategori";
  ...
  async getCustomerOverview() {
    ...
    return {
      totalCustomers,
      newCustomersThisMonth,
      newCustomersChange: this.calculateChange(
        newCustomersThisMonth,
        newCustomersLastMonth,
      ),
  ...
  async getCustomersByLocation() {
    ...
    for (const addr of addresses) {
      ...
      if (addr.province) {
        provinceMap.set(
          addr.province,
          (provinceMap.get(addr.province) || 0) + 1,
        );
      }
    }
   ...
   */
  private async getTopSellingProducts(limit = 10) {
    const orderItems = await this.db.orderItems.findMany({
      where: {
        variant_id: { not: null },
   ...
   */
  private async getTopRatedProducts(limit = 10) {
   ...
   */
  private async getMostReturnedProducts(limit = 10) {
    ...
    for (const item of returnItems) {
      if (!item.order_item?.variant?.product) continue;
  ...
  private getPeriodDates(period: "day" | "week" | "month" | "year", now: Date) {
    let startDate: Date;
